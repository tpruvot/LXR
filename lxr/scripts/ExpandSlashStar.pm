...
###############################################
#
# $Id: ExpandSlashStar.pm,v 1.1 2012/09/22 08:50:33 ajlittoz Exp $
#
...
##############################################################
#
#	Expand a file with C, PL/1, SQL style comments
#
##############################################################

# expand_slash_star takes 2 file arguments, $source and $dest.
...
# == Erasable comments ==
# Comments delimited /*- and -*/ are not copied to $dest.
# NOTE:	they may contain */ ... /* constructs, i.e. the non-comment
#		SQL ... text is also skipped. However, the /* and */ must
#		be properly balanced following SQL syntax.
...
# = Shell command: /*@X text */=
# Line is uncommented if generating a shell script (equivalent
# to a conditional block on option existence).
...
# 					$answer = get_user_choice($qtext, $qdeft, \@choices, \@answers);
	...
	#		existence test and fail instead of giving an error?
		...
		if ($line =~ m:^/\*\@case_C\s+(.*)\*/\s*\n:) {
			if ($1 ne $sel_choice) {	# Skip this alternative
				my $chskip = 0;
				while ($line = <$source>) {
					if (substr($line, 0, 10) eq "/*\@begin_C") {
						$chskip++;
						next;
					}
					if (substr($line, 0, 8) eq "/*\@end_C") {
						if ($chskip-- > 0) {
							next;
						} else {
							redo SCAN ;
...
sub skip_cond_block {
	...
	$end_line = substr($end_line, 0, 10);
	$end_line =~ s/begin/end/;
	$end_line = quotemeta ($end_line);
